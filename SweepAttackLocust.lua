function widget:GetInfo()
   return {
      name         = "SweepAttackLocust",
      desc         = "attempt to make Locust sweep area with flame to search for stealthy units. Version 1,00",
      author       = "terve886",
      date         = "2019",
...
local LocustUnitDefID = UnitDefNames["gunshipraid"].id

local cmdSweep = {
	id      = CMD_TOGGLE_SWEEP,
	type    = CMDTYPE.ICON,
	tooltip = 'Makes Locust sweep the area before it with attacks to search for stealthed units.',
	action  = 'oneclickwep',
	params  = { }, 
...
local SweeperController = {
	...
	new = function(self, unitID)
		...
		self = deepcopy(self)
		self.unitID = unitID
		self.range = (GetUnitMaxRange(self.unitID)-12)
	...
	sweep = function(self)
		...
		local targetPosRelative = {
			sin(heading+0.22*self.rotation)*(self.range),
			nil,
			cos(heading+0.22*self.rotation)*(self.range),
			}	
...
function widget:UnitFinished(unitID, unitDefID, unitTeam)
		if (UnitDefs[unitDefID].name==Locust_NAME)
...
function filterSweepers(units)
	...
	for i = 1, #units do
		...
		if (LocustUnitDefID == GetUnitDefID(unitID)) then
			n = n + 1
			filtered[n] = unitID
		end
	end
...
function widget:Initialize()
	...
	for i=1, #units do
		DefID = GetUnitDefID(units[i])
		if (UnitDefs[DefID].name==Locust_NAME)  then
