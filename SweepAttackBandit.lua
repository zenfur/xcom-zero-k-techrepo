function widget:GetInfo()
   return {
      name         = "SweepAttackBandit",
      desc         = "attempt to make Bandit sweep area with flame to search for stealthy units. Version 1,00",
      author       = "terve886",
      date         = "2019",
...
local BanditUnitDefID = UnitDefNames["shieldraid"].id

local cmdSweep = {
	id      = CMD_TOGGLE_SWEEP,
	type    = CMDTYPE.ICON,
	tooltip = 'Makes Bandit sweep the area before it with attacks to search for stealthed units.',
	action  = 'oneclickwep',
	params  = { }, 
...
local SweeperController = {
	...
	sweep = function(self)
		...
		local targetPosRelative = {
			sin(heading+0.25*self.rotation)*(self.range),
			nil,
			cos(heading+0.25*self.rotation)*(self.range),
			}	
...
function filterSweepers(units)
	...
	for i = 1, #units do
		...
		if (BanditUnitDefID == GetUnitDefID(unitID)) then
			n = n + 1
			filtered[n] = unitID
		end
	end
