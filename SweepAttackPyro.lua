function widget:GetInfo()
   return {
      name         = "SweepAttackPyro",
      desc         = "attempt to make Pyro sweep area with flame to search for stealthy units. Version 1,00",
      author       = "terve886",
      date         = "2019",
...
local cmdSweep = {
	id      = CMD_TOGGLE_SWEEP,
	type    = CMDTYPE.ICON,
	tooltip = 'Makes Pyro sweep the area before it with attacks to search for stealthed units.',
	action  = 'oneclickwep',
	params  = { }, 
...
function widget:UnitFinished(unitID, unitDefID, unitTeam)
		if (UnitDefs[unitDefID].name==Pyro_NAME)
...
function filterSweepers(units)
	...
	for i = 1, #units do
		...
		if (PyroUnitDefID == GetUnitDefID(unitID)) then
			n = n + 1
			filtered[n] = unitID
		end
	end
...
function widget:Initialize()
	...
	for i=1, #units do
		DefID = GetUnitDefID(units[i])
		if (UnitDefs[DefID].name==Pyro_NAME)  then
