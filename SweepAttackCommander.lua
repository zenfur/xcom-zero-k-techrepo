function widget:GetInfo()
   return {
      name         = "SweepAttackCommander",
      desc         = "attempt to make Commanders sweep area with flame to search for stealthy units. Version 1,00",
      author       = "terve886",
      date         = "2019",
	...
	sweep = function(self)
		...
		for rotationTest = 1, 3 do
			...
			local targetPosRelative = {
				sin(heading+0.18*self.rotation)*(self.range-10),
				nil,
				cos(heading+0.18*self.rotation)*(self.range-10),
				}	
			local testTargetPosRelative = {
				sin(heading+0.18*self.rotation)*(self.range-80),
				nil,
				cos(heading+0.18*self.rotation)*(self.range-80),
			}
			...
			local friendlies = GetUnitsInSphere(testTargetPosAbsolute[1], testTargetPosAbsolute[2], testTargetPosAbsolute[3], 167, self.allyTeamID)
